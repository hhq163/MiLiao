...
#define MIN_TIMER_DURATION	100	// 最小持续时间100 miliseconds
...
CEventDispatch::CEventDispatch()
{
	...
	m_kqfd = kqueue();
	if (m_kqfd == -1){
		Logger.Log(ERROR, "kqueue failed");
	}
#else
	m_epfd = epoll_create(1024);
	if (m_epfd == -1){
		Logger.Log(ERROR, "epoll_create failed");
	}
#endif
}
...
void CEventDispatch::AddEvent(SOCKET fd, uint8_t socket_event)
{
	...
	{
		Logger.Log(ERROR, "epoll_ctl failed, errno=%d", errno);
	}
}
