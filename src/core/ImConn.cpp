...
Logger.Log(INFO, "imconn_callback333");
	...
	case NETLIB_MSG_CLOSE:
		...
		break;
	default:
		Logger.Log(ERROR, "!!!imconn_callback error msg: %d ", msg);
		break;
	}
...
CImConn::CImConn()
{
	Logger.Log(INFO, "CImConn::CImConn");
...
CImConn::~CImConn()
{
	Logger.Log(INFO, "CImConn::~CImConn, handle=%d", m_handle);
}
...
//	++g_send_pkt_cnt;
	...
	while (remain > 0) {
		...
		Logger.Log(INFO, "CImConn::Send date length=%u ", len);

		int ret = netlib_send(m_handle, (char*)data + offset , send_size);
		if (ret <= 0) {
			Logger.Log(ERROR, "Err CImConn::Send date error, ret=%d", ret);
			ret = 0;
			break;
		}
	...
	if (remain > 0){
		...
		m_busy = true;
		Logger.Log(NOTIC,"send busy, remain=%d ", m_out_buf.GetWriteOffset());
	}else{
        OnWriteCompelete();
    }
...
void CImConn::OnRead()
{
	...
	uint16_t serviceId = CByteStream::ReadUint16(m_in_buf.GetBuffer() + 8);
	uint16_t commandId = CByteStream::ReadUint16(m_in_buf.GetBuffer() + 10);
	Logger.Log(INFO, "CImConn::OnRead2222, serviceId=%d, commandld=%d",serviceId,  commandId);
...
//			++g_recv_pkt_cnt;
		...
		Logger.Log(ERROR, "!!!catch exception, sid=%u, cid=%u, err_code=%u, err_msg=%s, close the connection ",
...
void CImConn::OnWrite()
{
	...
	Logger.Log(INFO, "onWrite, remain=%d ", m_out_buf.GetWriteOffset());
}
