...
/******************************************************************************
 * bool CLogger::Init(uint32_t dwID, uint32_t dwIP, uint16_t wPort)
 *
 * Parameter:
 * Input:
    uint32_t dwID, ID     uint32_t dwIP, IP     uint16_t wPort,端口
 * Output: 无
 * Returns:
    ==  false:   失败
    ==  true:    成功
...
/******************************************************************************
 * void CLogger::SetLogLevel(uint16_t wLevel, bool bSet)
 *
 * Parameter:
 * Input:
    uint16_t wLevel, 设置log级别
    bool bSet,
        enum LOG_LEVEL
        {
 ...
 * Parameter:
 * Input:
     const char *pFileName, 日志文件名
     unsigned int nLine,  插入行数
     LOG_LEVEL eLevel,  日志级别
     const char *pBuffer,  写入内容
     ...
 * Output:
 * Returns:
    ==  -1 写入出错
...
int CLogger::Write(LOG_INFO *pLogInfo)
{
    ...
    if (STAT_LEVEL == pLogInfo->eLevel){
        ...
        }else if (pLogTime->tm_hour != (int32_t)m_dwIndexS){
            fclose(m_pStatFile);
