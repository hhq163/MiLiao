...
CConfigFileReader* CDBManager::m_configFile = NULL;
...
bool CPrepareStatement::Init(MYSQL* mysql, string& sql)
{
	...
	if (!m_stmt) {
		Logger.Log(ERROR, "mysql_stmt_init failed");
		return false;
	}

	if (mysql_stmt_prepare(m_stmt, sql.c_str(), sql.size())) {
		Logger.Log(ERROR, "mysql_stmt_prepare failed: %s", mysql_stmt_error(m_stmt));
		return false;
	}
...
void CPrepareStatement::SetParam(uint32_t index, int& value)
{
	if (index >= m_param_cnt) {
		Logger.Log(ERROR, "index too large: %d", index);
		return;
	}
...
{
	if (!m_stmt) {
		Logger.Log(ERROR, "no m_stmt");
		return false;
	}

	if (mysql_stmt_bind_param(m_stmt, m_param_bind)) {
		Logger.Log(ERROR, "mysql_stmt_bind_param failed: %s", mysql_stmt_error(m_stmt));
		return false;
	}

	if (mysql_stmt_execute(m_stmt)) {
		Logger.Log(ERROR, "mysql_stmt_execute failed: %s", mysql_stmt_error(m_stmt));
		return false;
	}

	if (mysql_stmt_affected_rows(m_stmt) == 0) {
		Logger.Log(ERROR, "ExecuteUpdate have no effect");
		return false;
	}
...
int CDBConn::Init()
{
	...
	if (!m_mysql) {
		Logger.Log(ERROR, "mysql_init failed");
		return 1;
	}
	...
	if (!mysql_real_connect(m_mysql, m_pDBPool->GetDBServerIP(), m_pDBPool->GetUsername(), m_pDBPool->GetPasswrod(),
		...
		Logger.Log(ERROR, "mysql_real_connect failed: %s", mysql_error(m_mysql));
		return 2;
	}
...
CResultSet* CDBConn::ExecuteQuery(const char* sql_query)
{
	...
	if (mysql_real_query(m_mysql, sql_query, strlen(sql_query))) {
		Logger.Log(ERROR, "mysql_real_query failed: %s, sql: %s", mysql_error(m_mysql), sql_query);
		return NULL;
	}

	MYSQL_RES* res = mysql_store_result(m_mysql);
	if (!res) {
		Logger.Log(ERROR, "mysql_store_result failed: %s", mysql_error(m_mysql));
		return NULL;
	}
...
int CDBPool::Init()
{
	...
	Logger.Log(INFO, "db pool: %s, size: %d", m_pool_name.c_str(), (int)m_free_list.size());
	return 0;
}
...
CDBConn* CDBPool::GetDBConn()
{
	...
	while (m_free_list.empty()) {
		if (m_db_cur_conn_cnt >= m_db_max_conn_cnt) {
			...
			int ret = pDBConn->Init();
			if (ret) {
				Logger.Log(ERROR, "Init DBConnecton failed");
				delete pDBConn;
				...
				m_free_list.push_back(pDBConn);
				m_db_cur_conn_cnt++;
				Logger.Log(INFO, "new db connection: %s, conn_cnt: %d", m_pool_name.c_str(), m_db_cur_conn_cnt);
			}
		}
	}
...
CDBManager* CDBManager::getInstance(CConfigFileReader* configFile)
{
...
int CDBManager::Init()
{
	...
	char* db_instances = m_configFile->GetConfigName("DBInstances");

	if (!db_instances) {
		Logger.Log(ERROR, "not configure DBInstances");
		return 1;
	}
	...
	for (uint32_t i = 0; i < instances_name.GetItemCnt(); i++) {
		...
		char* db_host = m_configFile->GetConfigName(host);
		char* str_db_port = m_configFile->GetConfigName(port);
		char* db_dbname = m_configFile->GetConfigName(dbname);
		char* db_username = m_configFile->GetConfigName(username);
		char* db_password = m_configFile->GetConfigName(password);
        char* str_maxconncnt = m_configFile->GetConfigName(maxconncnt);

		if (!db_host || !str_db_port || !db_dbname || !db_username || !db_password || !str_maxconncnt) {
			Logger.Log(ERROR, "not configure db instance: %s", pool_name);
			return 2;
		}
		...
		if (pDBPool->Init()) {
			Logger.Log(ERROR, "init db instance failed: %s", pool_name);
			return 3;
		}
