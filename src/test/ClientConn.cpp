...
uint32_t ClientConn::reqGroupCreate(uint32_t nUserId)
{

    CImPdu cPdu;
    IM::Group::IMGroupCreateReq msg;
    msg.set_user_id(nUserId);
	msg.set_group_name("密聊开发群1");
	msg.set_group_type(IM::BaseDefine::GROUP_TYPE_TMP);
	msg.set_group_avatar("http://avatar.csdn.net/5/7/7/1_chenyufei1013.jpg");
	msg.add_member_id_list(930);
	msg.add_member_id_list(946);
	msg.add_member_id_list(944);

    cPdu.SetPBMsg(&msg);
    cPdu.SetServiceId(IM::BaseDefine::SID_GROUP);
    cPdu.SetCommandId(IM::BaseDefine::CID_GROUP_CREATE_REQUEST);
    uint32_t nSeqNo = m_pSeqAlloctor->getSeq(ALLOCTOR_PACKET);
    cPdu.SetSeqNum(nSeqNo);
    SendPdu(&cPdu);
    return nSeqNo;
...
void ClientConn::_HandleLoginResponse(CImPdu* pPdu)
{
    ...
    if(msgResp.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
    {
        ...
        Logger.Log(INFO, "ClientConn::_HandleLoginResponse nSeqNo = %d, nRet=%d, strMsg= %s", nSeqNo, nRet, strMsg.c_str());
        if(nRet == 0){
            m_bOpen = true;
